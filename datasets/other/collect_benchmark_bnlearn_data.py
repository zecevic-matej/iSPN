...
for interv in interventions:
    models.append({'model': model_obs['model'].copy(), 'adjmat': model_obs['adjmat'].copy(), 'interv': interv})
    intervene(models[-1], interv, intervention='uniform')
    if visualize_immediate:
        bn.plot(models[-1])
...
if save_dir:
    ...
    save_loc = os.path.join(save_dir, '{}_uniform_interventions_N{}.pkl'.format(bif_file.split('.bif')[0], N))
